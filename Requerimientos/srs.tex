...
La estructura de este documento sigue las recomendaciones de la \emph{guía
...
del producto, como la perspectiva de este y sus características principales.
\item \emph{Sección 3: }Describe las interfaces del producto, tanto las
del usuario como las interfaces de software. 
...
contradicciones con esta, para luego ser aceptado o mostrar las contradicciones
encontradas.
...
Respuesta de Daniel : Metas lógicas es mas de implementación mientras
que al usuario le interesa hacer preguntas.
\end{comment}
{}.
...
necesarias para la construcción de la extensión en sí.


\subsubsection{Optimización de la KB {*}}

Proveer la capacidad de optimizar la KB. Esta característica permite
la detección de redundancias y obtener una versión minimal de la KB
...
automática de redundancias no puedo asegurar que tenemos una KB minimal,
solo lo podre asegurar cuando estas no existan. Servirá seguir manteniendo
esta característica de la KB minimal o solo queremos quedarnos con
...
Respuesta de Daniel : No, solo vamos a realizar avisos de redundancias,
se deberá arreglar manualmente.
\end{comment}
{}
...
virología tales como lo que se encuentran en libros de estudio de
...
\item \href{http://books.google.com/books/about/Fields_virology.html?id=DMxcCOIGocUC}{Fields Virology - Bernard N. Fields, David Mahan Knipe}.
\end{itemize}
%
...
hecho sólo se utilizarán librerías compatibles con sistemas GNU/Linux.%
\begin{comment}
...
Provee un comando del linea a través del cual configurar y acceder
a la CLI de FuL, al cual el usuario le otorgara los siguientes parámetros
%
\begin{comment}
Pregunta : La KB deberíamos considerarla opcional ya que podríamos
...
También podríamos considerar opcional el de las extensiones ( que
...
Respuesta de Daniel : Ambas cosas serán opcionales, pero debe darse
un archivo de configuración aunque este este vació.
\end{comment}
{}:
\begin{itemize}
\item Un archivo de configuración XML donde se especifique el conjunto de
extensiones que FuL utilizará en esta ejecución. Además este archivo
puede contener configuración para las variables de FuL y las extensiones
a registrar.\\
Este parámetro es obligatorio, pero se considera valido un XML
que no presente configuración ni registro de extensiones alguna.
\item La KB inicial la cual FuL utilizara durante toda la sesión. Este parámetro
es opcional, en caso de no otorgarse FuL iniciara con una KB vacía.
\end{itemize}
A medida que se cargan dichos elementos se mostrarán por consola los
mensajes pertinentes al usuario, en caso de error se mostrará el mensaje
correspondiente y se anulará o continuará con su ejecución dependiendo
de la gravedad del error%
\footnote{La clasificación de nivel de gravedad de los errores esta TBD, la
cual será definida una vez de que estos sean clasificados y estudiados
...
la cual permite ejecutar los siguientes comandos%
...
}, FuL abortará el proceso de importación e informará al usuario mediante
la CLI del error con un nivel de detalle adecuado. Si la ejecución
del comando a sido exitosa se mostrará en la CLI el mensaje \emph{IMPORT
OK}.
...
a sido exitosa se mostrará en la CLI el mensaje \emph{SAVED}. En caso
de error informará al usuario mediante la CLI de dicho error.
...
obtenido y las lineas de razonamiento que se utilizaron para arribar
a el.%
...
llegar a una respuesta definida entonces FuL devolverá una de las
posibles listas de elementos requeridos para que FuL de una respuesta
definida.
...
por otro setting que tienes con otro script no hace falta tener que
...
Respuesta IMPORTANTE de Daniel : KISS \& YANGI
\end{comment}
{}
...
por FuL o las extensiones y \emph{value }un valor válido para esta.
...
variable}. En caso de que \emph{value} sea un valor inválido para
...
de que no se haya guardado la KB que FuL tiene en memoria, se mostrará
el mensaje \emph{WARNING: memory state not saved. Sure to quit? (y/n).}
Si el usuario lo confirma entonces se saldrá de la CLI y cerrará FuL,
sino abortará el comando.
\end{enumerate}
...
En caso de que el comando introducido no este registrado por las extensiones
...
Es el lenguaje que el usuario deberá utilizar para introducir conocimiento
y realizar consultas a FuL. También es con el cual se presentarán
los resultados de las operaciones solicitadas. Dicho lenguaje sera
capaz de representar conocimiento relacionado con el área de Virología%
\footnote{Pero no exclusivamente.%
}, el cual tendrá la propiedad de ser almacenable en una KB y ser interpretado
por una extensión.%
\begin{comment}
...
\item \label{ite::-FXP-}\href{http://code.google.com/p/fx-parser/}{FXP}
: Parser XML para C++, requiere \href{http://expat.sourceforge.net/}{eXpat}.
...
variables de entorno. Fácil de usar y altamente compatible con \href{http://STL}{STL}.
\item \href{http://code.google.com/p/mili/}{MiLi} : Colección de librerías
útiles de C++.
\item \label{ite::-unplugged-}\href{http://code.google.com/p/unplugged/}{unplugged}:
Librería que provee las herramientas genéricas necesarias para el
soporte de extensiones en Linux y Windows. 
\end{itemize}
...
\paragraph{Nomenclatura para los requerimientos}
\begin{itemize}
\item ERROR\_EXIT\emph{ :}\textbf{\emph{ La CLI deberá informar al usuario
de los errores ocurridos y se finalizará dicha operación. }}
\item \emph{<parámetro> : }Es un parámetro ingresado al comando de dicha
linea, siendo \emph{parámetro }el valor del mismo.
\end{itemize}

\subsection{\label{sub:REQ_01-Ejecutar-FuL}REQ\_01 Ejecutar FuL como command-line}
\begin{itemize}
...
las extensiones. Esta podrá tener una lista de las extensiones a agregar,
las cuales podrán estar acompañadas por una asignación de valores
a las variables que ellas otorgan al usuario para que este manipule.
Este parámetro es obligatorio.
\item \emph{database.kb : }La KB inicial con la cual FuL procesará los comandos
...
de FuL no presentes en dicho archivo tomarán el valor default definido
por FuL. Para esto deberá utilizarse FXP%
\footnote{Leer \vref{ite::-FXP-}.%
}. En caso de error ERROR\_EXIT.
...
de las extensiones la configuración de estás presente en el mismo
documento. También se registrarán los comandos que cada extensión
otorgue. Para esto se deberá utilizar unplugged%
\footnote{Leer \vref{ite::-unplugged-}.%
}. En caso de no encontrarse la configuración de dicha extensión, no
encontrarse la extensión propiamente dicha u algún otro tipo de error
ERROR\_EXIT.
\item \emph{Cargar la KB : }Se deberá procesar el conocimiento existente
en la KB por parte de FuL, teniendo en cuenta su configuración y la
...
Respuesta de Daniel : LET THE GALAXY BURN!!!!.
\end{comment}
{}.\\
...
los paso previos. Para los pasos posteriores leer \vref{sub:REQ_02-Ejecuci=0000F3n-de}.
\end{enumerate}
\end{itemize}

\subsection{\label{sub:REQ_02-Ejecuci=0000F3n-de}REQ\_02 Ejecución de comandos
en la CLI}
\begin{itemize}
\item \textbf{\emph{Objetivo :}} El objetivo de este requerimiento es permitirle
...
conjunto de comandos registrados por extensiones. Leer \vref{sub:REQ_01-Ejecutar-FuL}.%
} y la devolución de respuestas.
\item \textbf{\emph{Prioridad :}} Alta
\item \textbf{\emph{Descripción :}}\emph{ }

\begin{enumerate}
\item \emph{Introducir un comando : }El usuario podrá introducir un comando
...
}. Si así sucede se procederá a los siguientes pasos:
...
invalid parameters}.
\item Si $comm\,\epsilon\,\{"import","save","query","execute","show","set"\}$,
se procederá acorde al requerimiento correspondiente para dicho comando
\item Si no se cumple ninguno de los anteriores se procederá a deslindar
la ejecución de dicho comando a la extensión bajo la cual fue registrado%
\footnote{Leer \vref{sub:REQ_01-Ejecutar-FuL}.%
}, con los parámetros \emph{<p\_1> ... <p\_n>}.
\end{enumerate}
\end{enumerate}
\item \emph{Esperar : }Se esperara hasta que el usuario introduzca un nuevo
comando, en cuyo caso se volverá al paso 1.
\item \emph{Finalizar : }Si la KB que FuL tiene en memoria no fue guardada
en un archivo%
\begin{comment}
...
state not saved. Sure to quit? (y/n)}. Si el usuario elige \emph{y}
entonces se finaliza la ejecución de FuL, devolviendo al usuario a
la terminal desde donde se ejecuto el command-line de FuL. Si elije
\emph{n} entonces se volverá al paso 2.\\
...
\subsection{\label{sub:REQ_03}REQ\_03 Ejecución del comando IMPORT}
\begin{itemize}
\item \textbf{\emph{Objetivo :}} El objetivo de este requerimiento es permitirle
...
Respuesta de Daniel : Si, todo esto es implementación, tienes que
representarlo como una caja negra.

Mi Solución : Elimine todos los pasos que decían que se debía hacer
después de que y solo me quede con 2 pasos. No creo que este mal aclarar
que hay un paso de validación y otro de registrado ya que es algo
que queremos que se haga en ese orden para evitar que se registre
una parte de lo que hay en delta.kb cuando en otro punto podríamos
encontrar otro error. Por eso pienso que hace falta que haya 2 pasos
claros.
\end{comment}
{}
...
contendrá conocimiento, sintáctica y semánticamente valido, interpretable
por FuL o por las extensiones registradas en el. También se deberá
chequear que dicho conocimiento no es contradictorio con el presente
en la KB de FuL. De no cumplirse alguna de estas condiciones se deberá
devolver el mensaje de error correspondiente a través de la CLI y
se finaliza la ejecución del comando.
...
FuL, saturando el conjunto de elementos de la KB con conceptos atómicos
derivables del conocimiento presente en el archivo y la KB juntas.
Finalizado el registrado se deberá mostrar al usuario, mediante la
CLI, el mensaje \emph{IMPORT OK }y\emph{ }se finaliza la ejecución
del comando
\end{enumerate}
\end{itemize}

\subsection{\label{sub:REQ_04}REQ\_04 Ejecución del comando SAVE}
\begin{itemize}
\item \textbf{\emph{Objetivo :}} El objetivo de este requerimiento es permitirle
al usuario guardar el conocimiento presente en la KB en un archivo
\emph{<database.kb>}.
\item \textbf{\emph{Prioridad :}} Alta
\item \textbf{\emph{Descripción :}}\emph{ }
...
\subsection{\label{sub:REQ_05}REQ\_05 Ejecución del comando QUERY}
\begin{itemize}
\item \textbf{\emph{Objetivo :}} El objetivo de este requerimiento es permitirle
al usuario\emph{ }realizar una consulta\emph{ }a FuL a través de la
CLI, a la cual este dará una respuesta respecto de su KB y las extensiones
...
requerimos para poder demostrar, en un experimento o validación del
nuevo conocimiento, los cuales son las lineas de pensamiento que dieron
...
De no cumplirse alguna de estas condiciones se deberá devolver el
mensaje de error correspondiente a través de la CLI y se pasará al
paso final.
\item \emph{Procesado de la Consulta : }Se deberá procesar la consulta con
respecto al conocimiento presente en la KB de FuL y las extensiones
registradas.
...
\subsection{\label{sub:REQ_06}REQ\_06 Ejecución del comando EXECUTE}
\begin{itemize}
\item \textbf{\emph{Objetivo :}} El objetivo de este requerimiento es permitirle
...
uno de los comandos presentes en script.ful%
\footnote{Leer \vref{sub:REQ_02-Ejecuci=0000F3n-de}.%
}.
\item \emph{Finalizada la ejecución del comando : }Se finaliza la ejecución
del comando.
\end{enumerate}
...
\subsection{\label{sub:REQ_07}REQ\_07 Ejecución del comando SHOW}
\begin{itemize}
\item \textbf{\emph{Objetivo :}} El objetivo de este requerimiento es permitirle
...
el mensaje de error correspondiente a través de la CLI y se pasará
al paso final.
...
\subsection{\label{sub:REQ_08}REQ\_08 Ejecución del comando SET}
\begin{itemize}
\item \textbf{\emph{Objetivo :}} El objetivo de este requerimiento es permitirle
...
\item Las dependencias externas que puedan llegar a ser usadas en el producto
...
\footnote{Este último punto podría modificarse a futuro para utilizar licencias
más seguras para la protección de ideas.%
}. 
